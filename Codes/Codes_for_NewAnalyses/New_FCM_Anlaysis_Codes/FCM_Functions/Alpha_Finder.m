...
TrainSet = [2,10,23];
ValidSet = [1,11,22];
% [Alphas.LogXTrainL,Alphas.LogXTrainUL,Alphas.LogYTrainL,Alphas.LogXValidUL,Alphas.LogYValidL] = Collect_Data(NCellsL,NCellsUL,FeatureInds,TargetInd,TrainSet,ValidSet,DATA,QuadFeatures);
...
%% Which columns of S should be selected:
nrep = 1;
rng('default');
for i = 1:nrep*6
    if i <= nrep
