...
for Date = dates %For each of the dates experiments are run.
    for Un_Prefix = prefix_un
        for m=1:size(Un_Prefix,1)
            File_unstained{n,1} = [dir,Un_Prefix{m,1},' ',Date{1},' ',Species{1},' un.fcs.csv']; %Save unstained data in a ".csv" file.
...
for Date = dates %For each of the dates experiments are run.
    for St_Prefix = prefix_st
        for m=1:size(St_Prefix,1)
            File_stained{n,1} = [dir,St_Prefix{m,1},' ',Date{1},' ',Species{1},' st.fcs.csv']; %Save unstained data in a ".csv" file.
